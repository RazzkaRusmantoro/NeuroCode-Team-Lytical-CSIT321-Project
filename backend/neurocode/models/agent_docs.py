...
class AgentGuideTopicPointer(BaseModel):
    ...
    body: str = Field(
        ..., description="One sentence + instruction to load the rule file"
    )
    rule_path: str = Field(
        ..., description="Path to the rule .md, e.g. 'rules/subtitles.md'"
    )


class AgentGuideMetadata(BaseModel):
    tags: Optional[List[str]] = Field(
        default_factory=list, description="Tags for discovery"
    )
...
class AgentGuide(BaseModel):
    ...
    name: str = Field(
        ..., description="Slug for the filename, e.g. 'guide' (without .md)"
    )
...
class AgentRule(BaseModel):
    ...
    name: str = Field(
        ..., description="Slug for the filename, e.g. 'timing' (without .md)"
    )
    description: str = Field(..., description="Short description for frontmatter")
    metadata: Optional[AgentRuleMetadata] = None
    role: Optional[str] = Field(
        None, description="Optional: what is the role of this .md"
    )
