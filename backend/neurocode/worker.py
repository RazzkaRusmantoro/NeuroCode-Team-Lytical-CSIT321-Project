...
def index_job_result_ok(result: Dict[str, Any]) -> bool:

    rag = result.get("rag") or {}
    if rag.get("skipped"):
        pass
    elif rag.get("success") is False:
        return False
    kg = result.get("kg") or {}
    if kg.get("skipped"):
        pass
    elif kg.get("success") is False:
        return False
    return True


async def cleanup_after_failed_index_repo(
    *,
    ...
    branch: str,
    remove_branch_commits_doc: bool = False,
    analysis_directory: Optional[str] = None,
) -> None:

    print("[Index cleanup] Reverting partial index data...", flush=True)
    if analysis_directory:
        try:
            shutil.rmtree(analysis_directory, ignore_errors=True)
            print(
                f"[Index cleanup] Removed analysis dir: {analysis_directory}",
                flush=True,
            )
        except Exception as e:
            print(f"[Index cleanup] Could not remove analysis dir: {e}", flush=True)
    ...
    if organization_short_id and repository_name:
        try:
            ...
            print(f"[Index cleanup] Could not build collection name: {e}", flush=True)
    if collection_name:
        vdb = getattr(vectorizer, "vector_db", None)
        if vdb and hasattr(vdb, "delete_collection"):
    ...
    if repository_id:
        try:
            neo4j = Neo4jService()
            try:
                await neo4j.delete_graph(repository_id)
                print(
                    f"[Index cleanup] Neo4j graph cleared for repo_id={repository_id}",
                    flush=True,
                )
            finally:
                await neo4j.close()
        except Exception as e:
            print(f"[Index cleanup] Neo4j delete_graph failed: {e}", flush=True)

    if (
        remove_branch_commits_doc
        and mongodb_service
        and organization_id
        and repository_id
    ):
        r = mongodb_service.delete_repository_branch_commits(
            organization_id, repository_id
        )
        print(f"[Index cleanup] repository_branch_commits delete: {r}", flush=True)


async def run_index_repo_work(
    *,
    github_token: str,
    repo_full_name: str,
    branch: str = "main",
    ...
    print(
        f"[Worker] ▶ run_index_repo_work: {repo_full_name} branch={branch} repository_id={repository_id}",
        flush=True,
    )
    ...
    if organization_short_id and repository_name:
        _collection_name = build_collection_name(
            organization_name, organization_short_id, repository_name, branch
        )
        count = _qdrant_collection_count(_collection_name)
        if count > 0:
            print(
                f"[Worker] Qdrant collection '{_collection_name}' already has {count} vectors — "
    ...
    if repository_id:
        if await _neo4j_graph_exists(repository_id):
            ...
            print(
                f"[Worker] Building knowledge graph for repo_id={repository_id}...",
                flush=True,
            )
            try:
    ...
    print(
        f"[Worker] ✓ run_index_repo_work done: {repo_full_name} "
...
async def index_repo_job(
    ...
    *,
    ...
    return await run_index_repo_work(
        github_token=github_token,
        repo_full_name=repo_full_name,
        branch=branch,
        target=target,
...
async def build_kg_job(
    ...
    *,
    ...
    try:
        ...
        print(
            f"[Worker] ✓ build_kg_job done: success={result.get('success')}", flush=True
        )
        return result
    except Exception as e:
...
async def sync_docs_job(ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not list_result.get("success"):
        return {
            "success": False,
            "error": list_result.get("error", "Failed to list repos"),
        }
    ...
    for doc in repos:
        ...
        if not organization_id or not repository_id or not repo_full_name:
            continue
        token_result = mongodb_service.get_github_token_for_repo(
            organization_id, repository_id
        )
        if not token_result.get("success"):
            print(
                f"[Sync] No GitHub token for repo {repo_full_name}, skipping",
                flush=True,
            )
            continue
        ...
        if not names_result.get("success"):
            print(
                f"[Sync] Org/repo names not found for {repo_full_name}, skipping",
                flush=True,
            )
            continue
        ...
        for branch in branches_with_new_commits:
            ...
            try:
                ...
                print(
                    f"[Sync] index_pipeline failed {repo_full_name} branch={branch}: {e}",
                    flush=True,
                )
                continue
            ...
            if stored_sha:
                try:
                    changed_paths: List[str] = (
                        await github_fetcher.get_changed_file_paths(
            ...
            doc_list = (
                docs_result.get("documentations", [])
                if docs_result.get("success")
                else []
            )
            uml_list = (
                uml_result.get("uml_diagrams", []) if uml_result.get("success") else []
            )
            ...
            if pool:
                for did in affected_doc_ids:
                    await pool.enqueue_job(
                        "regenerate_documentation_job", documentation_id=did
                    )
...
async def startup(ctx: Dict[str, Any]) -> None:
    ...
    try:
        ...
        print(
            f"[Worker] Warning: could not create Redis pool for enqueue: {e}",
            flush=True,
        )
        ctx["redis_pool"] = None
    print("", flush=True)
    print("=" * 70, flush=True)
    print(
        "[Worker] ARQ worker started. Jobs: index_repo_job, sync_docs_job, regenerate_documentation_job, regenerate_uml_diagram_job.",
        flush=True,
    )
    print(f"[Worker] REDIS_URL = {_mask_redis_url(redis_url)}", flush=True)
    print("", flush=True)
    print(
        "  >>> If you add a repo and NOTHING appears here, the job is going to",
        flush=True,
    )
    print(
        "      a DIFFERENT Redis (e.g. your app is calling the deployed Python API).",
        flush=True,
    )
    print("      To see jobs in THIS terminal:", flush=True)
    print("        1. Run the Python API locally:  python run.py", flush=True)
    print(
        "        2. In Next.js .env set:  PYTHON_SERVICE_URL=http://localhost:8000",
        flush=True,
    )
    print(
        "        3. Use the SAME REDIS_URL for both API and this worker (e.g. localhost).",
        flush=True,
    )
    print("=" * 70, flush=True)
    print("", flush=True)
