...
class CodeAnalyzer:
    ...
    async def analyze_and_chunk(
        self, files: List[Dict[str, Any]], chunking_strategy: str = "hybrid"
    ) -> Dict[str, Any]:
        ...
        print(
            f"[CodeAnalyzer] Creating chunks with strategy: {chunking_strategy}...",
            flush=True,
        )
        ...
        return {
            ...
            "dependencies": self._format_dependencies(
                parse_result.structure.dependencies
            ),
            ...
            "metadata": {
                ...
                "totalFunctions": sum(
                    len(f.functions) for f in parse_result.structure.files
                ),
                "totalClasses": sum(
                    len(f.classes) for f in parse_result.structure.files
                ),
                "totalConstants": sum(
                    len(getattr(f, "constants", []))
                    for f in parse_result.structure.files
                ),
                "totalMethods": sum(
                    len(c.methods)
                    for f in parse_result.structure.files
                ...
                "totalRoutes": sum(
                    len(getattr(f, "routes", [])) for f in parse_result.structure.files
                ),
                "totalDefaultExports": sum(
                    len(getattr(f, "default_exports", []))
                    for f in parse_result.structure.files
                ),
            },
        }
    ...
    def _build_repository_structure(self, parse_result: ParseResult) -> Dict[str, Any]:
        ...
        return {
            "subsystems": [
                {"path": path, "files": sorted(files), "depth": 1}
    ...
    def _extract_symbols_summary(self, parse_result: ParseResult) -> Dict[str, Any]:
        ...
        for parsed_file in parse_result.structure.files:
            file_symbols = {
                ...
                "classes": [
                    {
                        "name": cls.name,
                        "methods": [
                            {"name": m.name, "parameters": len(m.parameters)}
                            for m in cls.methods
                        ],
                        "properties": [
                            {"name": p.name, "type": p.type} for p in cls.properties
                        ],
                        "extends": cls.extends,
                ...
                "imports": [
                    {"source": imp.source, "imports": imp.imports}
                    for imp in parsed_file.imports
                ],
                "exports": [
                    {"name": exp.name, "type": exp.type} for exp in parsed_file.exports
                ],
