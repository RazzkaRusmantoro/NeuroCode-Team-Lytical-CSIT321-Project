...
class TreeBuilder:
    ...
    def _safe_llm_call(
        self, prompt: str, max_tokens: int = 4000, retries: int = 3
    ) -> str:
        if not self.client:
            return ""
        for attempt in range(1, retries + 1):
            try:
                ...
                if attempt < retries and any(
                    k in msg.lower() for k in ("rate limit", "retry", "overloaded")
                ):
                    wait = 10.0
                    ...
                    if m:
                        try:
                            ...
                            pass
                    print(
                        f"[TreeBuilder] Rate limit; sleeping {wait:.1f}s (attempt {attempt}/{retries})"
                    )
                    time.sleep(wait)
                    continue
                if attempt == retries:
    ...
    def _safe_llm_call_fast(
        self, prompt: str, max_tokens: int = 3000, retries: int = 2
    ) -> str:
        if not self.client:
            return ""
        for attempt in range(1, retries + 1):
        ...
        def _ident(node):
            for ch in node.children:
                if ch.type in (
                    ...
                    return source_bytes[ch.start_byte : ch.end_byte].decode(
                        "utf-8", "ignore"
                    )
            return ""
    ...
    def _build_code_tree(self, files: List[Dict[str, Any]], repo_name: str):
        ...
        for f in files:
            ...
            file_node = TreeNode(
                name=parts[-1], type="file", path=rel_path, language=ts_lang or None
            )
            parent_node.children.append(file_node)

            if ts_lang:
                try:
                    symbols, imports, source_text = self._parse_content(
                        content, ts_lang, rel_path
                    )
        ...
        symbol_nodes = [
            n for n in all_nodes if n.type in ("function", "class") and n.name
        ]
    ...
    def _measure_min_depth(tree_list: list) -> int:
        ...
        def _depth(node: dict) -> int:
            ...
            if not children:
                return 0
            return (
                1 + min(_depth(c) for c in children if isinstance(c, dict))
                if children
                else 0
            )
    ...
    def _generate_feature_tree(
        ...
        readme_content: str = "",
        config_content: Optional[Dict[str, str]] = None,
    ) -> Optional[dict]:
        top_folders = sorted({p.split("/", 1)[0] for p in all_rel_files if "/" in p})[
            :80
        ]
...
{json.dumps(compact_info, ensure_ascii=False)}""".strip()
        ...
        try:
            ...
            print(
                f"[TreeBuilder] Pass 1 JSON parse failed: {e!r}, falling back to single-pass"
            )
        ...
        print(
            f"[TreeBuilder] Pass 1 complete — {len(tree_nodes)} domains, min depth to files: {min_depth}"
        )

        if min_depth < 3:
            print(
                f"[TreeBuilder] Hierarchy too shallow (depth {min_depth}), retrying with fallback"
            )
            return self._generate_feature_tree_fallback(
                repo_name,
                all_rel_files,
    ...
    def _generate_feature_tree_fallback(
        self,
        repo_name: str,
        all_rel_files: List[str],
        file_info: Dict[str, Dict],
        ...
        prompt = (
            f"""You are generating a DEEP, multi-level, feature-first repository map.
...
{json.dumps(file_data, ensure_ascii=False)}""".strip()
    ...
    def _apply_enrichment(self, file_node: TreeNode, fdata: dict):
        ...
        for child in file_node.children:
            ...
            child.dependencies = [
                str(x) for x in (sdata.get("dependencies") or []) if x
            ]
    ...
    def _prepare_file_data(self, fn: TreeNode, file_sources: Dict[str, str]) -> dict:
        ...
        return {
            "path": fn.path,
            "language": fn.language or "",
            "source": src,
            "symbols": syms,
        }

    def _enrich_files(
        self, code_root: TreeNode, file_sources: Dict[str, str], max_files: int = 999
    ):
        ...
        if failed:
            ...
            for fn in failed:
                ...
                try:
                    ...
                    print(
                        f"[TreeBuilder] Single file enrichment failed for {fn.path}: {e!r}"
                    )
    ...
    def _spec_to_tree_node(spec: dict) -> TreeNode:
        ...
        node = TreeNode(
            ...
            purpose=purpose,
            line_start=(
                int(line_start) if isinstance(line_start, (int, float)) else None
            ),
    ...
    def _gen_description(n: TreeNode, base: str, t: str, child_names: List[str]) -> str:
        ...
        if t == "code":
            return (
                "Complete file tree with all source code, folders, and parsed symbols."
            )
        if t == "file":
        ...
        if t == "function":
            loc = (
                f" at lines {n.line_start}-{n.line_end}"
    ...
    def _gen_explanation(n: TreeNode, base: str, t: str, child_names: List[str]) -> str:
        ...
        if t == "file":
            ...
            if classes:
                parts.append(
                    f"Defines {len(classes)} class(es): {', '.join(c.name for c in classes[:4])}."
                )
            if funcs:
                parts.append(
                    f"Contains {len(funcs)} function(s): {', '.join(f.name for f in funcs[:6])}."
                )
            return (
                f"{lang} source file. " + " ".join(parts)
                if parts
                else f"{lang} source file '{base}'."
            )
        if t == "function":
            loc = (
                f" (lines {n.line_start}-{n.line_end})"
                if n.line_start and n.line_end
                else ""
            )
    ...
    def build_tree(
        self, files: List[Dict[str, Any]], repo_name: str, readme_content: str = ""
    ) -> dict:
        ...
        if ai_data and isinstance(ai_data.get("tree"), list) and ai_data["tree"]:
            ...
            for seg in sorted(
                groups.keys(), key=lambda s: (s == "(root files)", s.lower())
            ):
                node = TreeNode(
                    name=seg, type="component", path="", description=f"Code area: {seg}"
                )
                for fp in sorted(groups[seg])[:60]:
                    src = file_lookup.get(fp)
                    node.children.append(
                        self._clone_subtree(src)
                        if src
                        else TreeNode(name=fp.rsplit("/", 1)[-1], type="file", path=fp)
                    )
                features_root.children.append(node)
        ...
        code_root_clone.description = (
            "Full file-tree view: folders, files, functions, classes."
        )
