...
class TreeSitterParser:
    ...
    async def parse_file(
        self, path: str, content: str, language: Optional[str] = None
    ) -> Optional[ParsedFile]:
        ...
        if not detected_lang or not is_language_supported(detected_lang):
            print(
                f'[TreeSitterParser] Unsupported language for {path}: {language or "unknown"}'
            )
            return None
        ...
        try:
            ...
            if not isinstance(grammar, Language):
                try:
                    ...
                    print(
                        f"[TreeSitterParser] Failed to convert grammar to Language: {e}"
                    )
                    return None
            ...
            default_exports = extract_default_exports(
                root_node, detected_lang, source_code
            )

            imports, dependencies = extract_imports(
                root_node, detected_lang, path, source_code
            )
            inheritance_deps = extract_inheritance(
                root_node, detected_lang, path, source_code
            )
    ...
    async def parse_files(self, files: List[Dict[str, Any]]) -> ParseResult:
        ...
        for file in files:
            try:
                parsed = await self.parse_file(
                    file["path"], file["content"], file.get("language")
                )
        ...
        for parsed_file in parsed_files:
            for func in parsed_file.functions:
                ...
                defined_functions_list.append(
                    {"name": func.name, "filePath": parsed_file.path}
                )
            ...
            for cls in parsed_file.classes:
                for method in cls.methods:
                    ...
                    defined_functions[full_name] = {
                        "name": full_name,
                        "filePath": parsed_file.path,
                    }
                    defined_functions_list.append(
                        {"name": full_name, "filePath": parsed_file.path}
                    )

                    defined_functions[method.name] = {
                        "name": method.name,
                        "filePath": parsed_file.path,
                    }
                    defined_functions_list.append(
                        {"name": method.name, "filePath": parsed_file.path}
                    )

        function_usage = build_usage_map(all_function_calls, defined_functions_list)

        call_dependencies = create_call_dependencies(
            all_function_calls, defined_functions
        )

        file_paths = [f["path"] for f in files]
        resolved_dependencies = self._resolve_dependencies(
            all_dependencies + call_dependencies, file_paths
        )

        structure = ParsedCodeStructure(
            files=parsed_files, dependencies=resolved_dependencies, modules=[]
        )
    ...
    def _resolve_dependencies(
        self, dependencies: List[Dependency], file_paths: List[str]
    ) -> List[Dependency]:
        ...
        for dep in dependencies:
            ...
            if (
                dep.to_path.startswith("/")
                or dep.to_path.startswith("http")
                or not dep.to_path.startswith(".")
            ):
                resolved.append(dep)
                continue

            from_dir = (
                "/".join(dep.from_path.split("/")[:-1]) if "/" in dep.from_path else ""
            )
            relative_path = dep.to_path

            resolved_path = self._resolve_relative_path(
                from_dir, relative_path, file_paths
            )
    ...
    def _resolve_relative_path(
        self, from_dir: str, relative_path: str, available_paths: List[str]
    ) -> Optional[str]:
        ...
        for potential_path in potential_paths:
            exact_match = next(
                (p for p in available_paths if p == potential_path), None
            )
            if exact_match:
                return exact_match

            without_ext = (
                potential_path.replace(".ts", "")
                .replace(".tsx", "")
                .replace(".js", "")
                .replace(".jsx", "")
            )
            match = next(
                (p for p in available_paths if p.startswith(without_ext)), None
            )
            if match:
                return match
