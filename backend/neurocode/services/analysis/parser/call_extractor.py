...
def extract_function_calls(
    root_node: Node, language: str, file_path: str, source_code: bytes
) -> List[FunctionCall]:
    ...
    if language in ("typescript", "tsx", "javascript", "jsx"):
        ...
        for node in _extract_nodes_by_type(root_node, "new_expression"):
            call = _extract_new_expression_from_node(
                node, file_path, root_node, source_code
            )
            if call:
                calls.append(call)
        ...
        for node in _extract_nodes_by_type(root_node, "call"):
            call = _extract_python_call_from_node(
                node, file_path, root_node, source_code
            )
            if call:
                calls.append(call)
...
def build_usage_map(
    calls: List[FunctionCall], defined_functions: List[Dict[str, str]]
...
def create_call_dependencies(
    calls: List[FunctionCall], defined_functions: Dict[str, Dict[str, str]]
) -> List[Dependency]:
...
def _extract_call_from_node(
    node: Node, file_path: str, root_node: Node, source_code: bytes
...
def _find_containing_function(
    current: Node, target: Node, source_code: bytes
) -> Optional[Node]:

    if (
        target.start_point[0] >= current.start_point[0]
        and target.end_point[0] <= current.end_point[0]
        and target.start_point[1] >= current.start_point[1]
        and target.end_point[1] <= current.end_point[1]
    ):
