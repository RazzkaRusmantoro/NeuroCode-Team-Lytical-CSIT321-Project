    ...
    if language in ("typescript", "tsx", "javascript", "jsx"):
        ...
        for node in _extract_nodes_by_type(root_node, "call_expression"):
            ...
            if require_stmt:
                imports.append(
                    ImportStatement(
                        source=require_stmt["source"], imports=["*"], isTypeOnly=False
                    )
                )
                dependencies.append(
                    Dependency(
    ...
    if language in ("typescript", "tsx", "javascript", "jsx"):
        for node in _extract_nodes_by_type(root_node, "class_declaration"):
            ...
            if superclass:
                ...
                class_name = (
                    _get_node_text(name_node, source_code) if name_node else None
                )
...
def _extract_import_from_node(
    ...
    return ImportStatement(source=source, imports=imports, isTypeOnly=is_type_only)
...
def _extract_python_import_from_node(node: Node, source_code: bytes) -> ImportStatement:
    ...
    return ImportStatement(
        source=source, imports=import_names if import_names else ["*"], isTypeOnly=False
    )


def _extract_python_from_import_from_node(
    node: Node, source_code: bytes
) -> ImportStatement:
    ...
    return ImportStatement(
        source=source, imports=imports if imports else ["*"], isTypeOnly=False
    )
