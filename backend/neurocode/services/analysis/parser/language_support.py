...
def detect_language(
    file_path: str, language_hint: Optional[str] = None
) -> Optional[str]:
    ...
    try:
        if lang_lower == "python" and tspython:
            ...
            if hasattr(tspython, "language"):
                grammar = (
                    tspython.language()
                    if callable(tspython.language)
                    else tspython.language
                )
            ...
            if hasattr(tsjavascript, "language"):
                js_grammar = (
                    ...
                    if callable(tsjavascript.language)
            ...
            if tstypescript:
                try:
                    if hasattr(tstypescript, "typescript") and callable(
                        tstypescript.typescript
                    ):
                        grammar = Language(tstypescript.typescript())
                    elif hasattr(tstypescript, "language") and callable(
                        tstypescript.language
                    ):
                        grammar = Language(tstypescript.language())
                    else:

                        ts_grammar = getattr(
                            tstypescript, "typescript", None
                        ) or getattr(tstypescript, "language", None)
                        if ts_grammar:
                            grammar = (
                                Language(ts_grammar)
                                if not isinstance(ts_grammar, Language)
                                else ts_grammar
                            )
                        else:
                            grammar = None
                ...
                try:
                    ...
                    if grammar:
                        ...
                        if lang_lower not in _grammar_cache:
                            print(
                                f"[LanguageSupport] TypeScript grammar not available, using JavaScript as fallback"
                            )
                except Exception:
                    grammar = None
            ...
            if tstypescript:
                try:
                    if hasattr(tstypescript, "tsx") and callable(tstypescript.tsx):
                        ...
                        if tsx_grammar:
                            grammar = (
                                Language(tsx_grammar)
                                if not isinstance(tsx_grammar, Language)
                                else tsx_grammar
                            )
                        else:
                            grammar = None
            ...
            if not grammar and tsjavascript:
                try:
                    ...
                    if grammar:
                        ...
                        if lang_lower not in _grammar_cache:
                            print(
                                f"[LanguageSupport] TSX grammar not available, using JavaScript as fallback"
                            )
                except Exception:
                    grammar = None
            ...
            if hasattr(tsjava, "language"):
                grammar = (
                    tsjava.language() if callable(tsjava.language) else tsjava.language
                )
            ...
            if hasattr(tsrust, "language"):
                grammar = (
                    tsrust.language() if callable(tsrust.language) else tsrust.language
                )
            ...
            if hasattr(tscpp, "language"):
                grammar = (
                    tscpp.language() if callable(tscpp.language) else tscpp.language
                )
