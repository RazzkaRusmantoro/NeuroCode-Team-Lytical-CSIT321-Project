...
def extract_functions(
    root_node: Node, language: str, source_code: bytes
    ...
    if language in ("typescript", "tsx", "javascript", "jsx"):
        ...
        for node in _extract_nodes_by_type(root_node, "variable_declaration"):
            for arrow_func in _extract_arrow_functions_from_declaration(
                node, language, source_code
            ):
                if arrow_func:
...
def extract_routes(
    ...
    if language in ("typescript", "tsx", "javascript", "jsx"):
        for node in _extract_nodes_by_type(root_node, "call_expression"):
            ...
            receiver = (
                _get_node_text(receiver_node, source_code) if receiver_node else None
            )
            routes.append(
                RouteDefinition(
                    path=path,
...
def extract_default_exports(
    root_node: Node, language: str, source_code: bytes
) -> List[DefaultExportDefinition]:
...
def _extract_arrow_functions_from_declaration(
    ...
    for child in node.named_children:
        if child.type != "variable_declarator":
            continue
        func = _extract_arrow_function_from_declarator(
            child, node, language, source_code
        )
        if func:
            results.append(func)
    return results
...
def _extract_arrow_function_from_declarator(
    ...
    parameters = _extract_parameters(
        value_node.child_by_field_name("parameters"), source_code
    )
...
def _extract_method_from_node(
    ...
    is_async = _has_modifier(node, "async", source_code)
    is_public = not _has_modifier(node, "private", source_code) and not _has_modifier(
        node, "protected", source_code
    )
...
def _extract_property_from_node(
    node: Node, source_code: bytes
    ...
    return PropertyDefinition(
        name=name, type=prop_type, isPublic=is_public, isStatic=is_static
    )


def _extract_python_function_from_node(
    node: Node, source_code: bytes
) -> Optional[FunctionDefinition]:
    ...
    name = _get_node_text(name_node, source_code)
    parameters = _extract_python_parameters(
        node.child_by_field_name("parameters"), source_code
    )
    body = _extract_function_body(node, source_code)
...
def _extract_python_class_from_node(
    node: Node, source_code: bytes
) -> Optional[ClassDefinition]:
...
def _extract_python_method_from_node(
    node: Node, source_code: bytes
) -> Optional[MethodDefinition]:
...
def _extract_parameters(
    params_node: Optional[Node], source_code: bytes
) -> List[Parameter]:
...
def _extract_python_parameters(
    params_node: Optional[Node], source_code: bytes
) -> List[Parameter]:
...
def _extract_parameter_from_node(node: Node, source_code: bytes) -> Optional[Parameter]:
    ...
    return Parameter(
        name=name, type=param_type, optional=optional, defaultValue=default_value
    )


def _extract_python_parameter_from_node(
    node: Node, source_code: bytes
) -> Optional[Parameter]:
