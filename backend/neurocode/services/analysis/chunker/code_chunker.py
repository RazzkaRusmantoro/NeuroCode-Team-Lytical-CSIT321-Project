...
from neurocode.services.analysis.chunker.models import (
    CodeChunk,
    ChunkMetadata,
    ChunkType,
)
...
class CodeChunker:
    ...
    def create_chunks(
        self, parse_result: Any, strategy: str = "hybrid"
    ) -> List[CodeChunk]:
    ...
    def _create_function_chunks(
        self, structure: ParsedCodeStructure, function_usage: Dict[str, FunctionUsage]
    ) -> List[CodeChunk]:
        ...
        for parsed_file in structure.files:
            ...
            for func in parsed_file.functions:
                ...
                calls = self._get_function_calls(
                    func.name, parsed_file.path, function_usage
                )
                called_by = self._get_called_by(
                    func.name, parsed_file.path, function_usage
                )
                dependencies = self._get_file_dependencies(
                    parsed_file.path, structure.dependencies
                )
            ...
            for i, default_exp in enumerate(
                getattr(parsed_file, "default_exports", [])
            ):
                ...
                if len(flow_functions) == 1:
                    ...
                    for flow_func_id in flow_functions:
                        ...
                        if flow_func and flow_file:
                            flow_file_content = self.file_contents.get(
                                flow_file.path, ""
                            )
                            ...
                            flow_content.append(
                                f"// {flow_file.path}:{flow_func.name}\n{func_code}"
                            )
                    ...
                    chunk = CodeChunk(
                        ...
                        related_chunks=[
                            f"{f['file']}:{f['function']}" for f in flow_metadata
                        ],
                    )
                    chunks.append(chunk)
    ...
    def _create_single_function_chunk(
        ...
        return CodeChunk(
            ...
            metadata=ChunkMetadata(
                ...
                imports=[imp.source for imp in parsed_file.imports],
                exports=[
                    exp.name for exp in parsed_file.exports if exp.name == func.name
                ],
                calls=calls,
                called_by=called_by,
                dependencies=dependencies,
    ...
    def _get_called_by(
        ...
        if function_name in function_usage:
            ...
            return [
                f"{call.filePath}:{call.context or 'unknown'}"
                for call in usage.calledIn
            ]
        return []

    def _get_file_dependencies(
        self, file_path: str, dependencies: List[Dependency]
    ) -> List[str]:
    ...
    def _find_function_by_id(
        self, func_id: str, structure: ParsedCodeStructure
        ...
        for chunk in chunks:
            ...
            for caller in chunk.metadata.called_by:
                if ":" in caller:
                    ...
                    for other_chunk in chunks:
                        if (
                            other_chunk.metadata.file_path == file_path
                            and other_chunk.metadata.function_name == func_name
                        ):
                            related.append(other_chunk.id)
