...
def run_semantic_clustering(
    ...
    print(
        f"[semantic_clustering] Embedding {len(snippets)} symbol nodes...", flush=True
    )
    try:
        ...
        embeddings = embed_model.encode(
            snippets, show_progress_bar=False, batch_size=32
        )
        import numpy as np
    ...
    cluster_label_map: Dict[int, str] = {
        cid: _heuristic_cluster_label(names) for cid, names in cluster_names.items()
    }
    ...
    for n in nodes:
        ...
        n["properties"]["semanticClusterLabel"] = cluster_label_map.get(
            cid, "Unclustered"
        )
