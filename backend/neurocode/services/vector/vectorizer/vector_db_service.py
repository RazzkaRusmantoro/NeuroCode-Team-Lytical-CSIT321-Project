...
class VectorDBService:
    ...
    def __init__(
        ...
        if url:
            ...
            if persist_directory:
                ...
                print(
                    f"[VectorDBService] Initializing local Qdrant at {persist_path}..."
                )
            ...
            if collection_name not in collection_names:
                ...
                if metadata:
                    try:
                        ...
                        metadata_point_id = uuid.uuid5(
                            uuid.NAMESPACE_DNS, f"{collection_name}_metadata"
                        )
                        metadata_point = PointStruct(
                            id=metadata_point_id,
                            vector=[0.0] * embedding_dimension,
                            payload={"type": "collection_metadata", **metadata},
                        )
                        self.client.upsert(
                            collection_name=collection_name, points=[metadata_point]
                        )
                        print(
                            f"[VectorDBService] Stored collection metadata: {metadata}"
                        )
                    except Exception as e:
                        print(
                            f"[VectorDBService] Warning: Could not store collection metadata: {e}"
                        )
    ...
    def add_chunks(
        ...
        print(
            f"[VectorDBService] Adding {len(chunks)} chunks to collection '{collection_name}'..."
        )
            ...
            point = PointStruct(
                ...
                payload={
                    ...
                    "keywords": (
                        ...
                        if isinstance(chunk["metadata"].get("keywords"), list)
                        else (chunk["metadata"].get("keywords") or "")
                    ),
                },
            )
            points.append(point)
        ...
        for i in range(0, len(points), batch_size):
            ...
            if (i + batch_size) % 100 == 0:
                print(
                    f"[VectorDBService] Added {min(i + batch_size, len(chunks))}/{len(chunks)} chunks..."
                )
    ...
    def delete_collection(self, collection_name: str) -> bool:
        ...
        try:
            ...
            print(f"[VectorDBService] delete_collection({collection_name!r}): {e}")
            return False
    ...
    def get_collection_metadata(self, collection_name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            result = self.client.retrieve(
                collection_name=collection_name, ids=[metadata_point_id]
            )
