...
class Vectorizer:
    ...
    def vectorize_chunks(
        ...
        print(
            f"[Vectorizer] Vectorizing {len(chunks)} chunks for collection '{collection_name}'..."
        )
        ...
        embeddings = self.embedding_service.embed_batch(
            texts_to_embed, batch_size=32, show_progress=True
        )
        ...
        if metadata:
            embedding_dimension = (
                len(embeddings[0])
                if embeddings
                else self.embedding_service.get_dimension()
            )
            self.vector_db.get_or_create_collection(
                collection_name, embedding_dimension, metadata=metadata
            )
