...
class S3Service:
    ...
    def generate_onboarding_path_key(
        self, organization_id: str, path_id: str, file_extension: str = "json"
    ) -> str:

        return f"organizations/{organization_id}/onboarding/paths/{path_id}.{file_extension}"

    def upload_documentation(
        self, content: str, s3_key: str, content_type: str = "application/json"
    ) -> dict:
    ...
    def get_documentation(self, s3_key: str) -> dict:
        ...
        try:
            ...
            if error_code == "NoSuchKey":
                return {"success": False, "error": f"Documentation not found: {s3_key}"}
            return {
                "success": False,
                "error": f"AWS S3 error: {error_code}",
                "details": str(e),
            ...
            return {"success": False, "error": f"Failed to retrieve from S3: {str(e)}"}
    ...
    def delete_documentation(self, s3_key: str) -> dict:
        ...
        try:
            ...
            return {"success": True, "message": f"Documentation deleted: {s3_key}"}
            ...
            return {"success": False, "error": f"Failed to delete from S3: {str(e)}"}
    ...
    def check_connection(self) -> dict:
        ...
        try:
            ...
            return {"success": False, "error": f"Failed to connect to S3: {str(e)}"}
