...
class MongoDBService:
    ...
    def __init__(self):
        ...
        try:
            ...
            print(
                f"[MongoDBService] ✓ Connected to MongoDB database: {self.database_name}"
            )
    ...
    def upsert_code_reference(
        ...
        try:
            ...
            return {"success": False, "error": f"MongoDB operation failed: {str(e)}"}
        except Exception as e:
            return {
                "success": False,
    ...
    def delete_repository_branch_commits(
        self,
        organization_id: str,
        repository_id: str,
    ) -> Dict[str, Any]:
        ...
        try:
            try:
                ...
                return {"success": False, "error": f"Invalid ObjectId format: {str(e)}"}
            result = self.db.repository_branch_commits.delete_one(
                {"organizationId": org_obj_id, "repositoryId": repo_obj_id}
            )
    ...
    def get_existing_uml_titles_descriptions(
    ...
    def _chat_doc_to_response(
        self, doc: Dict[str, Any], chat_id: Optional[str] = None
    ) -> Dict[str, Any]:
    ...
    def list_chats_by_user(
        self, user_id: str, context_id: Optional[str] = None
    ) -> Dict[str, Any]:
                ...
                "chat": self._chat_doc_to_response(
                    {
                        **doc,
                        "messages": messages,
                        "title": update["$set"].get("title", doc.get("title")),
                        "updatedAt": now,
                    },
                    chat_id,
                ),
