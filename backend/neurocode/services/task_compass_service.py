...
def _filter_collections(
    collections: List[str], repo_url_names: Optional[List[str]]
) -> List[str]:
    ...
    try:
        async with httpx.AsyncClient(timeout=15.0) as client:
            ...
            if resp.status_code != 200:
                print(
                    f"[task-compass] GitHub contributors API returned {resp.status_code} for {repo_full_name}"
                )
                return []
...
async def analyze_task(
    ...
    collection_names = vectorizer.vector_db.list_collections_by_org_short_id(
        org_short_id
    )
