...
Each path: title, summaryDescription (1-2 sentences), modules (7-10 items: name, summaryDescription, order). Do not invent wikis or hubs. Return only a JSON object: {{"paths": [{{"title":"...","summaryDescription":"...","modules":[{{"name":"...","summaryDescription":"...","order":1}},...]}},...]}}"""
        ...
        for p in paths_raw:
            ...
            for idx, m in enumerate(
                sorted(
                    modules_raw,
                    key=lambda x: (x if isinstance(x, dict) else {}).get("order", 999),
                )[:10]
            ):
...
Generate the JSON object with "documentation" (description + sections) and "code_reference_ids" (array). One section per module; section id "1", "2", ...; title = module name; description = thorough onboarding content from the code. Output only JSON."""
        ...
        if not json_match:
            return {
                "documentation": {"description": path_summary, "sections": []},
                "code_reference_ids": [],
            }
        try:
        ...
        return {
            "documentation": doc,
            "code_reference_ids": parsed.get("code_reference_ids", []),
        }
    ...
    def generate_documentation(
        ...
        result = self.generate_structured_documentation(
            prompt, context_chunks, repo_name
        )
    ...
    def generate_structured_documentation(
        ...
        schema_path = (
            Path(__file__).parent.parent / "config" / "documentation_schema.json"
        )
        schema_template = ""
...
"""
        other_repos_note = (
            _other_repos_instruction()
            if any(c.get("_collection") for c in context_chunks)
            else ""
        )
...
- If content is long, prioritize structure completeness over detail"""
        ...
        try:
            ...
            def extract_partial_data(json_str):
                ...
                try:
                    ...
                    code_refs_match = re.search(
                        r'"code_references"\s*:\s*\[(.*?)\]', json_str, re.DOTALL
                    )
                    if code_refs_match:
                    ...
                    glossary_match = re.search(
                        r'"glossary_terms"\s*:\s*\[(.*?)\]', json_str, re.DOTALL
                    )
                    if glossary_match:
                    ...
                    sections_match = re.search(
                        r'"sections"\s*:\s*\[(.*?)(?:\]|$)', json_str, re.DOTALL
                    )
                    if sections_match:
                        sections_content = sections_match.group(1)

                        section_objects = re.findall(
                            r'\{\s*"id"\s*:\s*"([^"]+)"\s*,\s*"title"\s*:\s*"([^"]+)"',
                            sections_content,
                        )
                        if section_objects:
                            sections = []
            ...
            if json_match:
                ...
                try:
                    ...
                    print(
                        f"[LLMService] Error at position: {e.pos if hasattr(e, 'pos') else 'unknown'}"
                    )
                    ...
                    try:
                        ...
                        code_refs = parsed.get("code_references", [])
                        code_ref_ids = [
                            ref for ref in code_refs if isinstance(ref, str)
                        ]

                        print(
                            f"[LLMService] Successfully fixed control characters in JSON"
                        )
                        return {
                        ...
                        try:
                            ...
                            print(
                                f"[LLMService] Successfully fixed truncated JSON with control character sanitization"
                            )
                            return {
                                "documentation": parsed.get(
                                    "documentation", {"sections": []}
                            ...
                            print(
                                f"[LLMService] Could not fix JSON, attempting partial extraction: {fix_error}"
                            )

                            partial_data = extract_partial_data(json_str)
                            print(
                                f"[LLMService] Extracted partial data: {len(partial_data['code_reference_ids'])} refs, {len(partial_data['documentation']['sections'])} sections"
                            )
                            return partial_data
            else:

                print(f"[LLMService] No JSON object found in response")
                return {"documentation": {"sections": []}, "code_reference_ids": []}
    ...
    def generate_architecture_documentation(
        ...
        schema_path = (
            Path(__file__).parent.parent / "config" / "architecture_schema.json"
        )
        schema_template = ""
        if schema_path.exists():
            try:
...
Produce a simple flowchart (nodes + edges) that explains this section in 2–6 steps. For each node include a short "description" (1–2 sentences) and a "role" (input, process, decision, or output). Return only the JSON object."""
        ...
        try:
            ...
            edges = [
                ...
                for e in edges
                if str(e.get("source")) in edge_ids and str(e.get("target")) in edge_ids
            ]
            ...
            return {
                "nodes": [
                    {
                        "id": "0",
                        "label": section_title[:50] or "Section",
                        "description": "",
                        "role": "process",
                    }
                ],
    ...
    def generate_agent_docs_bundle(
        ...
        schema_path = (
            Path(__file__).parent.parent / "config" / "agent_docs_bundle_schema.json"
        )
        schema_template = ""
        if schema_path.exists():
            try:
                ...
                print(
                    f"[LLMService] Warning: Could not load agent docs bundle schema: {e}"
                )
...
Return ONLY the JSON object (no ```json wrapper, no explanation)."""
        ...
        try:
            ...
            if json_match:
                ...
                if "guide" in parsed and "rules" in parsed:
                    return parsed
            return {
                "error": "Failed to parse agent docs bundle JSON",
                "guide": None,
                "rules": [],
            }
        except Exception as e:
    ...
    def generate_uml_class_diagram(
        ...
        schema_path = (
            Path(__file__).parent.parent / "config" / "uml_class_diagram_schema.json"
        )
        schema_template = ""
        if schema_path.exists():
            try:
...
Produce your best UML class diagram: clear, conceptual (e.g. GitHub, Visual Tree, Vector DB, Request, Service). CRITICAL: No central hub—do not have one class with 4 or more connections. Every class must have at most 3 relationships total. Every class must have at least one relationship—no isolated nodes; each class must appear as source or target in at least one relationship. For each class, capture as many attributes and methods as you can from the code (without making nodes too long); if the code does not specify enough, use guesswork. Also provide: (1) an "explanation" for each class; (2) a brief "description" for each attribute and each method. Ensure every relationship source and target is a class id from the classes array. Include sourceMultiplicity and targetMultiplicity for association, aggregation, and composition (e.g. "1", "0..1", "1..*", "*")."""
        ...
        try:
            ...
            classes = [
                ...
                for c in classes
                if isinstance(c, dict)
                and (c.get("id") or c.get("className")) in connected_ids
            ]
    ...
    def generate_uml_sequence_diagram(
        ...
        schema_path = (
            Path(__file__).parent.parent / "config" / "uml_sequence_diagram_schema.json"
        )
        schema_template = ""
        if schema_path.exists():
            try:
...
Output ONLY the JSON object."""
        ...
        try:
            ...
            if len(lifelines) < 2:
                return {
                    "error": "At least 2 lifelines required",
                    "lifelines": lifelines,
                    "messages": [],
                    "steps": [],
                    "fragments": fragments,
            ...
            if lifelines:
                has_destroy = any(
                    isinstance(ll, dict) and ll.get("isDestroyed") for ll in lifelines
                )
                if not has_destroy:
    ...
    def generate_uml_use_case_diagram(
        ...
        schema_path = (
            Path(__file__).parent.parent / "config" / "uml_use_case_diagram_schema.json"
        )
        schema_template = ""
        if schema_path.exists():
            try:
...
Produce a UML use case diagram. For each actor set placement: "left" for primary (User, Admin, Customer) or "right" for systems (Database, Payment Gateway). Each use case must have at most 3 relationships. systemBoundary, actors, useCases, relationships. Output ONLY the JSON object."""
        ...
        try:
            ...
            actor_ids = {
                a.get("id") or a.get("label") for a in actors if isinstance(a, dict)
            }
            for a in actors:
                if isinstance(a, dict):
                    ...
                    if a.get("placement") not in ("left", "right"):
                        a["placement"] = "left"
            use_case_ids = {
                u.get("id") or u.get("label") for u in use_cases if isinstance(u, dict)
            }
            for u in use_cases:
            ...
            def use_case_degree(uc_id: str, rels: list) -> int:
                return sum(
                    1
                    for x in rels
                    if x.get("source") == uc_id or x.get("target") == uc_id
                )
            ...
            def rels_for_use_case(uc_id: str, rels: list) -> list:
                return [
                    r
                    for r in rels
                    if r.get("source") == uc_id or r.get("target") == uc_id
                ]
            ...
            for uc_id in use_case_ids:
                ...
                ucr_sorted = sorted(
                    ucr, key=lambda r: _priority.get(r.get("relationship"), 0)
                )
            ...
            for aid in all_ids:
                ...
                for r in relationships:
                    ...
                    if (
                        src not in all_ids
                        or tgt not in all_ids
                        or (src != aid and tgt != aid)
                    ):
                        continue
                    rel_type = (r.get("relationship") or "communication").lower()
                    if rel_type not in (
                        ...
                        rel_type = "communication"
                    valid_rels.append(
                        {"source": src, "target": tgt, "relationship": rel_type}
                    )
                    connected.add(aid)
                    break
    ...
    def generate_uml_state_diagram(
        ...
        schema_path = (
            Path(__file__).parent.parent / "config" / "uml_state_diagram_schema.json"
        )
        schema_template = ""
        if schema_path.exists():
            try:
...
Produce a UML state diagram. One initial state, one or more final states, states, optional compositeStates (use at least one when the flow has a subgroup), and transitions. For events with parameters use eventParameter so they display as "event (parameter)". Output ONLY the JSON object."""
        ...
        try:
            ...
            final_id_set = {
                f.get("id") for f in final_states if isinstance(f, dict) and f.get("id")
            }
...
Produce a unique, detailed title and description for this diagram. Output ONLY the JSON object with "title" and "description"."""
    ...
    def generate_parameter_descriptions_batch(
        self, parameters: List[Dict[str, Any]], code_context: str
    ) -> Dict[str, str]:
            ...
            if response.content and len(response.content) > 0:
                ...
                if json_match:
                    try:
                        ...
                        print(
                            f"[LLMService] Failed to parse parameter descriptions JSON"
                        )
                ...
                return {
                    p["name"]: f"The {p['name'].replace('_', ' ')} parameter."
                    for p in parameters
                }
...
Do not include any other text, just the JSON object."""
        ...
        try:
            ...
            if response.content and len(response.content) > 0:
                ...
                if json_match:
                    try:
                        ...
                        print(
                            f"[LLMService] Failed to parse code reference descriptions JSON"
                        )
                ...
                for ref in code_references:
                    ...
                    if match:
                        ...
                        if ref_type == "class":
                            descriptions[ref_name] = (
                                "A class that provides functionality for managing operations and state within the application."
                            )
                        elif ref_type == "function":
                            descriptions[ref_name] = (
                                "A function that processes input data and returns transformed output according to its implementation."
                            )
                        else:
                            descriptions[ref_name] = (
                                "A method that performs operations on the class instance."
                            )
            ...
            print(
                f"[LLMService] Error generating batch code reference descriptions: {e}"
            )
