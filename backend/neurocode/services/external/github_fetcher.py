...
def _source_priority(path: str) -> int:
    ...
    if path_lower.startswith("src/"):
        return 0
    if (
        path_lower.startswith("app/")
        or path_lower.startswith("lib/")
        or path_lower.startswith("server/")
    ):
        return 1
...
class GitHubFetcher:
    ...
    async def fetch_repository_files(
        ...
        token_preview = (
            ...
            else ("(empty)" if not access_token else "(present)")
        )
        print(
            f"[GitHubFetcher] fetch_repository_files: repo={repo_full_name!r} branch={branch!r} token={token_preview}",
            flush=True,
        )
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            ...
            if not commit_sha and branch in ("main", "master"):
                ...
                print(
                    f"[GitHubFetcher] Trying fallback branch {fallback!r}.", flush=True
                )
                commit_sha = await self._get_branch_sha(
            ...
            if not commit_sha:
                print(
                    f"[GitHubFetcher] No commit SHA for branch {branch!r}; cannot fetch tree.",
                    flush=True,
                )
                return files
            print(
                f"[GitHubFetcher] Branch SHA: {commit_sha[:12]}... (branch={branch!r})",
                flush=True,
            )
    ...
    async def _get_branch_sha(
        ...
        try:
            ...
            print(
                f"[GitHubFetcher] _get_branch_sha {response.status_code} {url!r}: {body}",
                flush=True,
            )
        except Exception as e:
    ...
    async def _get_tree_recursive(
        ...
        try:
            ...
            if commit_response.status_code != 200:
                print(
                    f"[GitHubFetcher] _get_tree_recursive commit response {commit_response.status_code}: {commit_response.text[:300]}",
                    flush=True,
                )
                return []
            ...
            if not tree_sha:
                print(
                    f"[GitHubFetcher] _get_tree_recursive: no tree.sha in commit response",
                    flush=True,
                )
                return []
            ...
            if tree_response.status_code != 200:
                print(
                    f"[GitHubFetcher] _get_tree_recursive tree response {tree_response.status_code}: {tree_response.text[:300]}",
                    flush=True,
                )
                return []
            ...
            if tree_response.status_code == 200:
                ...
                for item in all_items:
                    ...
                    if file_path.startswith(".") or any(
                        skip in file_path
                        for skip in ["node_modules", "dist", "build", ".git"]
                    ):
                        continue
                ...
                filtered.sort(
                    key=lambda item: (
                        _source_priority(item.get("path", "")),
                        item.get("path", ""),
                    )
                )
                limited = filtered[: self.max_files]
                print(
                    f"[GitHubFetcher] Tree: {len(all_items)} items -> {len(filtered)} source files -> returning {len(limited)}",
                    flush=True,
                )
                if len(filtered) > self.max_files:
                    print(
                        f"[GitHubFetcher] Capping at {self.max_files} files (repo has {len(filtered)} source files). Set INDEX_MAX_FILES to index more.",
                        flush=True,
                    )
                return limited
    ...
    async def _batch_fetch_file_contents(
        ...
        for item in tree_items[: self.max_files]:
            ...
            if sha:
                tasks.append(
                    self._fetch_blob_content(
                        client, repo_full_name, access_token, sha, path
                    )
                )
