...
def build_collection_name(
    ...
    if not organization_short_id or not repository_name:
        raise ValueError(
            "organization_short_id and repository_name are required for collection naming"
        )
...
async def run_index_pipeline(
    *,
    ...
    if not branch or branch.strip().lower() in ("main", "master"):
        resolved_branch = await github_fetcher.get_default_branch(
    ...
    if chunks_list:
        ...
        for c in chunks_list:
            ...
            if fp not in by_file:
                by_file[fp] = []
            by_file[fp].append(
                c.get("metadata", {}).get("function_name")
                or c.get("metadata", {}).get("class_name")
                or "?"
            )
        ...
        for fp in sorted(by_file.keys()):
            symbols = by_file[fp]
            _log(
                f"  {fp}: {len(symbols)} chunks ({', '.join(symbols[:5])}{'...' if len(symbols) > 5 else ''})"
            )
    ...
    if chunks_list and llm_service:
        ...
        try:
            llm_service.enrich_chunks_for_retrieval(
                chunks_list, batch_size=8, max_content_chars=450
            )
    ...
    if vectorization_result.get("success"):
        ...
        _log(
            f"✓ Total in collection: {vectorization_result.get('total_in_collection', 'N/A')}"
        )
    else:
    ...
    return {
        ...
        "vectorization": (
            vectorization_result if vectorization_result.get("success") else None
        ),
