...
from neurocode.routes.documentation import (
    _agent_bundle_to_documentation,
    _get_other_repo_chunks,
)
...
async def regenerate_documentation(documentation_id: str) -> Dict[str, Any]:
    ...
    if not names_result.get("success"):
        return {
            "success": False,
            "error": names_result.get("error", "Org/repo not found"),
        }
    try:
    ...
    repo_name = (
        names_result.get("repo_full_name")
        or names_result.get("repository_name")
        or "repository"
    )
    ...
    try:
        ...
        if not search_results:
            return {
                "success": False,
                "error": f"No chunks in collection '{collection_name}'",
            }
        ...
        if doc_type == "aiAgent":
            ...
            if raw_bundle.get("error"):
                return {
                    "success": False,
                    "error": raw_bundle.get("error", "Agent bundle failed"),
                }
            ok, bundle, err = validate_agent_docs_bundle(raw_bundle)
            if not ok or bundle is None:
                return {
                    "success": False,
                    "error": err or "Agent bundle validation failed",
                }
        ...
        if s3_service and s3_key:
            ...
            if not s3_result.get("success"):
                return {
                    "success": False,
                    "error": s3_result.get("error", "S3 upload failed"),
                }

        mongodb_service.clear_documentation_sync_flags(
            documentation_id, file_paths=file_paths
        )
        return {"success": True}
...
async def regenerate_uml_diagram(diagram_id: str) -> Dict[str, Any]:
    ...
    if not diagram_result.get("success"):
        return {
            "success": False,
            "error": diagram_result.get("error", "Diagram not found"),
        }
