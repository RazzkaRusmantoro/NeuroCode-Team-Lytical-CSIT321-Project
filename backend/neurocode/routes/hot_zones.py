...
def _filter_collections_by_repo_url_names(
    collections: List[str], repo_url_names: List[str]
) -> List[str]:
...
async def recommend_areas(request: HotZonesRecommendAreasRequest) -> dict:
    if not vectorizer:
        raise HTTPException(
            status_code=503, detail="Vector search service not initialized"
        )
    ...
    collections = _filter_collections_by_repo_url_names(
        all_collections, request.repo_url_names or []
    )
    ...
    for coll in collections:
        try:
            ...
            if count == 0:
                continue
            hits = vectorizer.search(
                collection_name=coll, query=query, top_k=per_coll_k
            )
            for h in hits:
                h["_collection"] = coll
                all_hits.append(h)
