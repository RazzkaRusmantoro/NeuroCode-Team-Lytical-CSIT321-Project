...
async def list_chats(
    ...
    context_id: str | None = Query(
        None,
        description="Scope to a documentation context (e.g. repo-doc:org:repo, onboarding:org:pathSlug)",
    ),
) -> dict:
    ...
    if not result.get("success"):
        raise HTTPException(
            status_code=500, detail=result.get("error", "Failed to list chats")
        )
...
async def get_chat(
    ...
    if not result.get("success"):
        raise HTTPException(
            status_code=404, detail=result.get("error", "Chat not found")
        )
    return result
...
async def create_chat(body: CreateChatRequest) -> dict:
    ...
    if not result.get("success"):
        raise HTTPException(
            status_code=500, detail=result.get("error", "Failed to create chat")
        )
...
async def send_message(chat_id: str, body: SendMessageRequest) -> dict:
    ...
    if not client:
        raise HTTPException(
            status_code=503, detail="Chat not configured (OPENAI_API_KEY not set)"
        )
    ...
    if not get_result.get("success"):
        raise HTTPException(
            status_code=404, detail=get_result.get("error", "Chat not found")
        )
    ...
    try:
        ...
        reply = (
            (response.choices[0].message.content if response.choices else None) or ""
        ).strip() or "No response."
    except Exception as e:
        raise HTTPException(status_code=502, detail=f"OpenAI request failed: {e}")

    title_if_first = (
        message[:36].strip() + ("…" if len(message) > 36 else "") if message else None
    )
    ...
    if not append_result.get("success"):
        raise HTTPException(
            status_code=500,
            detail=append_result.get("error", "Failed to save messages"),
        )
    return {
        "reply": reply,
