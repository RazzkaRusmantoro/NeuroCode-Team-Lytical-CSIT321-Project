...
async def generate_visual_tree(request: GenerateVisualTreeRequest):
    ...
    try:
        ...
        has_token = bool(
            getattr(request, "github_token", None)
            and (request.github_token or "").strip()
        )
        print(
            f"github_token: {'present' if has_token else 'MISSING or empty'}",
            flush=True,
        )
        print("=" * 60, flush=True)
        ...
        if len(files) == 0:
            print(
                "[VisualTree] No files returned. Check logs above for GitHubFetcher (branch SHA, tree, token).",
                flush=True,
            )
            return {
                "success": False,
        ...
        repo_display_name = (
            request.repository_name or request.repo_full_name.split("/")[-1]
        )

        if tree_builder is None:
            raise HTTPException(
                status_code=503, detail="Tree builder service not available"
            )
