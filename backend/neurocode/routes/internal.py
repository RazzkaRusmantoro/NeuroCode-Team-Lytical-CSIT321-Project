...
from neurocode.models.schemas import (
    QueueIndexRequest,
    QueueKGBuildRequest,
    UpdateRepoBranchCommitsRequest,
)
from neurocode.worker import (
    cleanup_after_failed_index_repo,
    enqueue_build_kg,
    enqueue_index_repo,
    index_job_result_ok,
    run_index_repo_work,
...
def _use_inline_index() -> bool:

    if os.getenv("INDEX_USE_ARQ", "").strip().lower() in ("1", "true", "yes"):
        return False
    v = os.getenv("INDEX_INLINE", "").strip().lower()
    if v in ("0", "false", "no", "off", "arq", "queue"):
        return False
    return True
...
async def queue_index(
    request: QueueIndexRequest, _: None = Depends(require_internal_key)
):

    print(
        f"[queue-index] Received request: repo_full_name={request.repo_full_name} repository_id={request.repository_id}"
    )

    commits_doc_written = False
    if mongodb_service and request.organization_id and request.repository_id:
        try:
            ...
            if branch_commits:
                upsert_res = mongodb_service.upsert_repository_branch_commits(
                    organization_id=request.organization_id,
                    repository_id=request.repository_id,
                    branch_latest_commits=branch_commits,
                    repo_full_name=request.repo_full_name,
                )
                commits_doc_written = bool(upsert_res.get("success"))
                print(
                    f"[queue-index] Stored branch commits for {len(branch_commits)} branch(es)"
                )
            else:
                print(
                    f"[queue-index] No branches returned from GitHub for {request.repo_full_name}"
                )
        except Exception as e:
            print(
                f"[queue-index] Failed to store branch commits (continuing to enqueue): {e}"
            )
    ...
    if _use_inline_index():
        print(
            "[queue-index] INDEX_INLINE: running index on API process (no ARQ)",
            flush=True,
        )
        try:
            result = await run_index_repo_work(
            ...
            if not index_job_result_ok(result):
                await cleanup_after_failed_index_repo(
                    organization_id=request.organization_id,
                    repository_id=request.repository_id,
                    organization_short_id=request.organization_short_id,
                    organization_name=request.organization_name,
                    repository_name=request.repository_name,
                    branch=branch,
                    remove_branch_commits_doc=commits_doc_written,
                )
                raise HTTPException(
                    status_code=500,
                    detail="Index or knowledge-graph step failed; partial vectors/graph/DB rows were removed.",
                )
            print(
                f"[queue-index] Inline index finished repo_full_name={request.repo_full_name}",
                flush=True,
            )
            return {"enqueued": False, "inline": True, "result": result}
            ...
            print(f"[queue-index] Inline index error: {e}", flush=True)
            await cleanup_after_failed_index_repo(
                organization_id=request.organization_id,
                repository_id=request.repository_id,
                organization_short_id=request.organization_short_id,
    ...
    if job_id is None:
        print(
            f"[queue-index] Failed to enqueue: repo_full_name={request.repo_full_name}"
        )
        raise HTTPException(status_code=503, detail="Failed to enqueue index job")
    print(
        f"[queue-index] Enqueued job_id={job_id} repo_full_name={request.repo_full_name} repository_id={request.repository_id}"
    )
    return {"enqueued": True, "job_id": job_id}


@router.post("/queue-kg-build")
async def queue_kg_build(
    request: QueueKGBuildRequest, _: None = Depends(require_internal_key)
):
    ...
    try:
        ...
        if already_built:
            print(
                f"[queue-kg-build] Graph already exists in Neo4j — skipping.",
                flush=True,
            )
    ...
    try:
        ...
        if already_queued:
            print(
                f"[queue-kg-build] Build already in-flight — skipping duplicate.",
                flush=True,
            )
...
async def update_repo_branch_commits(
    ...
    if not result.get("success"):
        raise HTTPException(
            status_code=500,
            detail=result.get("error", "Failed to store branch commits"),
        )
    return {
        "success": True,
        "branch_latest_commits": branch_commits,
