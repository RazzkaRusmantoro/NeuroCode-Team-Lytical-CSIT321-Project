...
async def generate_suggested_paths(request: OnboardingSuggestedPathsRequest):
    ...
    for repo in request.repositories:
        ...
        if not organization_short_id or not repository_name:
            _log(
                f"[onboarding] Skip {repo_full_name}: missing org_short_id or repository_name"
            )
            continue
        ...
        try:
            ...
            if existing == 0:
                ...
                if not result.get("success"):
                    _log(
                        f"[onboarding] Index failed for {repository_name}: {result.get('message')}"
                    )
                ...
                _log(
                    f"[onboarding] Using existing collection for {repository_name} ({existing} chunks)"
                )
...
def _path_doc_query(path) -> str:
    ...
    for m in sorted(path.modules, key=lambda x: x.order):
        parts.append(m.name)
        parts.append(
            getattr(m, "summary_description", None)
            or getattr(m, "summaryDescription", "")
            or ""
        )
...
async def generate_path_doc(request: GenerateOnboardingPathDocRequest):
    ...
    if not llm_service:
        raise HTTPException(
            status_code=503, detail="LLM service not available. Set ANTHROPIC_API_KEY."
        )
    if not s3_service:
    ...
    modules_for_llm = [
        {
            "name": m.name,
            "summary_description": getattr(m, "summary_description", None)
            or getattr(m, "summaryDescription", ""),
            "order": m.order,
        }
        for m in sorted(path.modules, key=lambda x: x.order)
    ...
    _log(
        f"[onboarding path-doc] Generating doc for '{path.title}' with {len(all_chunks)} chunks"
    )
    ...
    if not s3_result.get("success"):
        raise HTTPException(
            status_code=500, detail=s3_result.get("error", "S3 upload failed")
        )
