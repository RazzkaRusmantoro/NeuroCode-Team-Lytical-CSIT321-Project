...
async def analyze_pull_request(request: AnalyzePullRequestRequest):
    ...
    try:
        ...
        if request.organization_short_id and request.repository_name:
            org_name_safe = sanitize_name(
                request.organization_name or request.organization_short_id
            )
            ...
            collection_name = (
                f"{org_name_safe}_{org_slug_safe}_{repo_name_safe}_{branch}"
            )
            ...
            try:
                ...
                if query_text.strip():
                    search_results = vectorizer.search(
                        collection_name=collection_name, query=query_text, top_k=5
                    )
        ...
        traceback.print_exc()
        raise HTTPException(
            status_code=500, detail=f"Failed to analyze pull request: {str(e)}"
        )
...
async def generate_pr_analysis(
    pr_data: Dict, changed_files: List[Dict], related_code: List[Dict]
) -> Dict:
            ...
            try:
                ...
                if analysis.get("issues"):
                    ...
                    for issue in analysis.get("issues", []):
                        ...
                        if issue.get("severity") == "low" and issue_type_lower in [
                            "style",
                            "formatting",
                            "naming",
                        ]:
                ...
                if analysis.get("description", {}).get("architecturalImplications"):
                    if isinstance(
                        analysis["description"]["architecturalImplications"], str
                    ):

                        analysis["description"]["architecturalImplications"] = {
                            "approach": analysis["description"][
                                "architecturalImplications"
                            ],
                            "benefits": [],
                            "systemEvolution": "",
                ...
                elif not analysis.get("description", {}).get(
                    "architecturalImplications"
                ):
                    analysis["description"]["architecturalImplications"] = {}
                ...
                if analysis.get("issues") and len(analysis.get("issues", [])) > 0:
                    ...
                    if analysis.get("description", {}).get("overallAssessment"):
                        if isinstance(
                            analysis["description"]["overallAssessment"], dict
                        ):
                            if analysis["description"]["overallAssessment"].get(
                                "issuesSummary"
                            ):
                                analysis["description"]["overallAssessment"][
                                    "issuesSummary"
...
def extract_code_snippet(
    file_path: str, line_number: int, changed_files: List[Dict], context_lines: int = 5
) -> str:
            ...
            for i, line in enumerate(lines):
                ...
                if line.startswith("@@"):
                    ...
                    if match:
                        ...
                        in_relevant_hunk = (
                            hunk_start <= target_line <= (hunk_start + hunk_count + 20)
                        )
                        if in_relevant_hunk:
                    ...
                    if comment.get("body") and comment.get("path"):
                        ...
                        if comment_path and comment_line is not None:
                            code_snippet = extract_code_snippet(
                                comment_path, comment_line, changed_files
                            )
...
def calculate_risk_assessment(
    changed_files: List[Dict], pr_data: Dict, issues: List[Dict] = None
) -> Dict:
    ...
    if critical_files:
        factors.append(
            f"Critical infrastructure files modified ({len(critical_files)})"
        )
        risk_score += 20
    ...
    if issues:
        ...
        for i in issues:
            ...
            if any(
                keyword in issue_file
                for keyword in ["schema", "config", "prompt", "template"]
            ):

                if (
                    "will break" in issue_expl
                    or "break" in issue_desc
                    or "crash" in issue_expl
                    or "fail" in issue_expl
                ):
                    ...
                    if (
                        "migration" in issue_expl
                        or "backwards compatibility" in issue_expl
                        or "validation" in issue_expl
                    ):
        ...
        if actual_critical_issues:
            factors.append(
                f"{len(actual_critical_issues)} critical breaking issue(s) detected"
            )
            risk_score += 30
        if high_breaking_issues:
            factors.append(
                f"{len(high_breaking_issues)} high-severity breaking issue(s) detected"
            )
            risk_score += 15
